...
public class ModifyStateActionDialog extends QDialog
{
	private State state;
	...
	public ModifyStateActionDialog(State state)
	{
		this.state = state;
	...
	{		
		...
		{
			state.getOnentry().set(entry_actions.currentIndex(), action_string.displayText());
		}
		else if (actions_types.checkedId() == 1)
		{
			state.getDuring().set(during_actions.currentIndex(), action_string.displayText());
		}
		else
		{
			state.getOnexit().set(exit_actions.currentIndex(), action_string.displayText());
		}
